...
class RelayCubit extends Cubit<RelayState> {
  ...
  Future loadData(model.Relay relay) async {
    relayStatus = relay.relay == 'active';
    sw = relay.status == 'ON';
    timer = relay.timer == 'active';
    infinity = relay.startDate == '11/11/11' && relay.endDate == '11/11/11';
    startdate = Jalali(
        int.parse(relay.startDate.split('/')[0]),
        int.parse(relay.startDate.split('/')[1]),
        int.parse(relay.startDate.split('/')[2]));
    enddate = Jalali(
        int.parse(relay.endDate.split('/')[0]),
        int.parse(relay.endDate.split('/')[1]),
        int.parse(relay.endDate.split('/')[2]));
    newRelayState = relay;
    ...
    try {
      ...
      switch (currentPage) {
        case Page.Relay1:
          ...
          loadData(deviceStatus.getR1);
          break;

        case Page.Relay2:
          loadData(deviceStatus.getR2);
...
//sms send
  ...
  relay() async {
    ...
    sendSMS(relayStatusVar == false ? '$pageNumber:off#' : '$pageNumber:on#',
        onPressed: () async {
  ...
  currentSensor() async {
    ...
    sendSMS(sensorVar ? '$pageNumber:CA#' : '$pageNumber:CD#',
        onPressed: () async {
      sensor = sensorVar;
      ...
      if (currentPage == Page.Relay1)
        deviceStatus.getPublicReport.currentSensor1 = current;
      else if (currentPage == Page.Relay3)
        deviceStatus.getPublicReport.currentSensor3 = current;
      else if (currentPage == Page.Relay6)
        deviceStatus.getPublicReport.currentSensor6 = current;
  ...
  humidityStatus() {
    ...
    sendSMS(
        '$pageNumber:rl:${sw == false ? 'd' : 'a'},time:${timer == false ? 'd' : 'a'},hu:${humidityVar ? 'a' : 'd'}#',
        onPressed: () {
      ...
      ///because humidity only in relay 3
      deviceBox.put('info', deviceStatus);
      constants.put(
          'IR2',
          humidity
              ? 'assets/selectable-icons/fan.png'
              : 'assets/selectable-icons/question.png');
  ...
  lightStatus() async {
    ...
    sendSMS(
        '$pageNumber:rl:${sw == false ? 'd' : 'a'},time:${timer == false ? 'd' : 'a'},lu:${lightVar ? 'a' : 'd'}#',
        onPressed: () async {
      light = lightVar;
  ...
  humudityAct() {
    ...
    sendSMS('h-min:$sv,max:$ev#',
        showDialog: false); //run operation if humidity is active
  ...
  void relay7knob() {
    sv = deviceStatus.getR7.lux.isNotEmpty
        ? int.parse(deviceStatus.getR7.lux)
        : 1;
    start = KnobController(
        minimum: 1,
        maximum: 50,
        initial: deviceStatus.getR7.lux.isNotEmpty
            ? double.parse(deviceStatus.getR7.lux).clamp(1, 50)
            : 1);

    dialog('Set light', m.StatefulBuilder(builder: (context, setState) {
              ...
              child: m.SizedBox(
                ...
                child: Knob(
                  ...
                  style: const KnobStyle(
                    labelStyle: m.TextStyle(color: m.Colors.transparent),
                    controlStyle: ControlStyle(
                        tickStyle:
                            ControlTickStyle(color: m.Colors.transparent),
                    ...
                    minorTickStyle:
                        MinorTickStyle(color: m.Color(0xffaaadba), length: 6),
                    majorTickStyle:
                        MajorTickStyle(color: m.Color(0xffaaadba), length: 6),
                  ),
                ),
              ),
            ),
  ...
  relay3knob() {
    endMin = 0;
    sv = deviceStatus.getR3.humMin.isNotEmpty
        ? int.parse(deviceStatus.getR3.humMin)
        : 5;
    ev = deviceStatus.getR3.humMin.isNotEmpty
        ? int.parse(deviceStatus.getR3.humMax)
        : 9;
    start = KnobController(
        minimum: 5, //0
        maximum: 95, //95
        initial: endMin!.clamp(
            deviceStatus.getR3.humMin.isNotEmpty
                ? double.parse(deviceStatus.getR3.humMin)
                : 5,
            95));
    end = KnobController(
        ...
        maximum: 95, //95
        initial: endMin!.clamp(
            deviceStatus.getR3.humMax.isNotEmpty
                ? double.parse(deviceStatus.getR3.humMax)
                : 9,
            95));

    dialog('Set humidity', m.StatefulBuilder(builder: (context, setState) {
      start!.addOnValueChangedListener((double value) {
        setState(() {
          ...
          ev = endMin!.toInt(); //value.roundToDouble();
            ...
            minimum: endMin!,
            maximum: 95, //double.parse(deviceStatus.getR3.humMax),
            initial: endMin!.clamp(
                endMin!, 95)); //double.parse(deviceStatus.getR3.humMax)));
      });
          ...
          child: m.SizedBox(
            ...
            child: Knob(
              ...
              style: const KnobStyle(
                labelStyle: m.TextStyle(color: m.Colors.transparent),
                controlStyle: ControlStyle(
                    tickStyle: ControlTickStyle(color: m.Colors.transparent),
                    glowColor: m.Colors.transparent,
                    backgroundColor: m.Color(0xffdde6e8),
                    shadowColor: m.Color(0xffd4d6dd)),
                pointerStyle: PointerStyle(color: blue),
